...
if [ -n "$RID" ] && [ -f "state/triage/dispatch-$RID.json" ]; then
  for iid in $(jq -r '.items[].item_id' "state/triage/dispatch-$RID.json"); do
    ...
    if grep -Fxq "$iid" state/block-acks.txt 2>/dev/null; then
      python3 yaas-triage/ack-watch.py ack "$RID" "$iid" blocked "fixture blocked" >/dev/null
      continue
    fi
    python3 yaas-triage/ack-watch.py ack "$RID" "$iid" handled "fixture ack" >/dev/null
  done
fi
...
python3 "$TRIAGE/ensure-watch-ids.py" quest-test "$QUEST/watch.json"
...
python3 "$TRIAGE/ensure-watch-ids.py" quest-contract "$CONTRACT_QUEST/watch.json"
