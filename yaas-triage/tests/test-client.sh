...
spec = importlib.util.spec_from_file_location("client", f"{sys.argv[1]}/client.py")
...
run() { python3 "$SCRIPT_DIR/client.py" "$@" >/dev/null 2>&1; echo $?; }
eq "no args"                  "$(run)"                          "3"
...
for shim in mcp-call.sh jira-call.sh slack-react.sh; do
  if [ -x "$SCRIPT_DIR/$shim" ]; then ok "$shim still present and executable"
  else bad "$shim is missing — worker instructions reference it by name"; fi
done
eq "mcp-call.sh forwards bad args"  "$(bash "$SCRIPT_DIR/mcp-call.sh" 2>/dev/null; echo $?)" "3"
eq "jira-call.sh forwards bad args" "$(bash "$SCRIPT_DIR/jira-call.sh" 2>/dev/null; echo $?)" "3"
