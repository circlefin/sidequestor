...
mkdir -p "$RIG" "$LOGS"
cp "$SCRIPT_DIR/run-agent.py" "$SCRIPT_DIR/format-stream.py" "$RIG/"
...
fake_agent 'printf "%s\n" "{\"a\":1}"; sleep 60'
BEFORE=$(pgrep -f "sleep 60" | wc -l | tr -d " ")
START=$(date +%s)
...
sleep 1
AFTER=$(pgrep -f "sleep 60" | wc -l | tr -d " ")
...
grep -q "run-agent.py" "$SCRIPT_DIR/manual-dispatch.sh" && ok "manual-dispatch.sh uses it" \
