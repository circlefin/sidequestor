...
REPO="$TMP/repo"
mkdir -p "$REPO/yaas-triage/checkers" "$REPO/state/quests/active/q1"
cp "$SCRIPT_DIR/approval-helper.py" "$REPO/yaas-triage/"
...
A() { python3 yaas-triage/approval-helper.py "$@"; }
...
grep -q 'TERMINAL_APPROVAL_STATUSES = ("executed", "cancelled")' "$SCRIPT_DIR/dashboard-server.py" \
  && ok "terminal statuses are named in one place" || bad "no TERMINAL_APPROVAL_STATUSES constant"
if grep -q 'status") in ("pending_review", "needs_reply")' "$SCRIPT_DIR/dashboard-server.py"; then
...
eq "every filter site uses the shared constant" \
   "$(grep -c 'TERMINAL_APPROVAL_STATUSES' "$SCRIPT_DIR/dashboard-server.py")" "5"
