...
mkdir -p "$TRI" "$REPO/state/quests/active/q-demo"
cp "$SCRIPT_DIR/slack-send.py" "$SCRIPT_DIR/approval-helper.py" "$TRI/"
...
EOF
  chmod +x "$TRI/mcp-call.sh"
  : > "$TMP/sent.log"
}

send() {  # send <extra_json_fields>
  ( cd "$REPO" && REPO_ROOT="$REPO" python3 "$TRI/slack-send.py" \
...
mk_slack "$EDGE_UNDER"
OUT=$( cd "$REPO" && REPO_ROOT="$REPO" YAAS_STALE_REPLY_HOURS=1 python3 "$TRI/slack-send.py" \
...
mk_slack "$FRESH"
OUT=$( cd "$REPO" && REPO_ROOT="$REPO" YAAS_FORCE_DRAFT=1 python3 "$TRI/slack-send.py" \
       '{"quest_id":"q-demo","channel_id":"C1","message":"hi","thread_ts":"1785000000.000100"}' 2>&1 )
printf '%s' "$OUT" | grep -q '"held": true' \
