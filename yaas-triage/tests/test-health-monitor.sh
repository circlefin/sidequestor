...
MON() { python3 "$SCRIPT_DIR/health-monitor.py" --repo "$REPO" --json "$@" 2>/dev/null; }
...
python3 "$SCRIPT_DIR/health-monitor.py" --repo "$REPO" >/dev/null 2>&1 \
...
python3 "$SCRIPT_DIR/health-monitor.py" --repo "$REPO" --notify >/dev/null 2>&1 || true
FIRST=$(wc -l < "$REC" | tr -d ' ')
python3 "$SCRIPT_DIR/health-monitor.py" --repo "$REPO" --notify >/dev/null 2>&1 || true
...
if grep -qE "^import fcntl|fcntl\.|flock" "$SCRIPT_DIR/health-monitor.py"; then
...
if grep -qE "subprocess\.[a-z]+\(\[[^]]*(triage|dispatch-agent|mcp-call)" "$SCRIPT_DIR/health-monitor.py"; then
...
if grep -qE "^\s*import (requests|yaml|jq)|^\s*from (requests|yaml)" "$SCRIPT_DIR/health-monitor.py"; then
