"""The public command shell used by the editable Stage 3 package."""
...
def _usage() -> str:
    lines = ["usage: yaas [--workspace PATH] COMMAND [ARGS...]", "", "commands:"]
...
def _command_help(command: str) -> str:
    examples = {
        "init": "yaas init PATH [--name NAME]",
        "instances": "yaas instances list|doctor|register PATH|rekey PATH",
        "setup": "yaas --workspace PATH setup --render-only|install|status|uninstall",
        "tick": "yaas --workspace PATH tick [--dry-run|--isolated [--fake-worker]]",
        "loop": "yaas --workspace PATH loop [--max-ticks N]",
        "dashboard": "yaas --workspace PATH dashboard serve|url",
    }
    usage = examples.get(command, f"yaas --workspace PATH {command} [ARGS...]")
...
def _cmd_init(args: list[str]) -> int:
    ...
    print(f"initialized YAAS workspace: {workspace.root}")
...
def _cmd_migrate(path: str | None, name: str | None = None) -> int:
    ...
    if changed:
        print(f"migrated YAAS workspace: {workspace.root}")
        if archive:
