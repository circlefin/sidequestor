...
def prune_approvals(state: Path):
    """Drop long-finished approvals.

    Takes the SAME sidecar lock as ledger/approval-helper.py and ops/dashboard-server.py.
    This previously wrote with no lock at all, so a prune could race a reviewer clicking
    approve and silently drop that transition. Every writer of this file must agree on one
    lock, and it cannot be the data file itself because the writes replace its inode.
    """
    path = state / "pending-approvals.json"
    if not path.exists():
        return
    lock_path = state / "pending-approvals.json.lock"
    with open(lock_path, "a+") as lockf:
        fcntl.flock(lockf, fcntl.LOCK_EX)
        try:
            _prune_approvals_locked(path)
        finally:
            fcntl.flock(lockf, fcntl.LOCK_UN)


def _prune_approvals_locked(path: Path):
    try:
