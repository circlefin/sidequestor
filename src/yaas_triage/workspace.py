...
def _read_json(path: Path) -> dict:
    try:
        ...
        raise SystemExit(f"invalid YAAS workspace marker: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise SystemExit(f"invalid YAAS workspace marker: {path}: expected object")
    return value
...
def init_workspace(path: str | Path, name: str | None = None) -> Workspace:
    ...
    if not (root / ".env.example").exists():
        (root / ".env.example").write_text("# YAAS workspace configuration\n")
...
def load_workspace(path: str | Path) -> Workspace:
    ...
    if not marker.exists():
        raise SystemExit(f"not a YAAS workspace: {root} (run 'yaas init {root}')")
